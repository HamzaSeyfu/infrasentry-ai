...
def main() -> int:
    ...
    query = f'''\n    mutation CreateLinkedInDraft {{\n      createPost(input: {{\n        text: {graphql_string(text)}\n        channelId: "{channel_id}"\n        schedulingType: automatic\n        mode: addToQueue\n        saveToDraft: true\n      }}) {{\n        ... on PostActionSuccess {{\n          post {{\n            id\n            text\n            status\n          }}\n        }}\n        ... on MutationError {{\n          message\n        }}\n      }}\n    }}\n    '''

    data = graphql(query)
    result = data.get("createPost") or {}

    if result.get("message"):
        raise RuntimeError(f"Buffer rejected the draft: {result['message']}")

    post = result.get("post")
    if not post:
        raise RuntimeError(f"Unexpected Buffer response: {json.dumps(result)}")

    print(f"Buffer LinkedIn draft created: id={post['id']} status={post.get('status')}")
    return 0
